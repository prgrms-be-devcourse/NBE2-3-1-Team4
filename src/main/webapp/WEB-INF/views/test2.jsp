    ...
    <script type="text/javascript">
        window.onload = function () {
            ...
            productItems.forEach((productItem) => {
                const productName = productItem.querySelector('.row:nth-child(2)').innerText.trim(); // 상품 이름 가져오기
            ...
            // 상품 개수 초기화 확인
            console.log(productQuantities); // 상품 이름별로 개수가 0으로 초기화된 객체 출력
            console.log(productPrices); // 상품 가격 출력

            // 모든 상품 아이템에 대해 "추가" 및 "감소" 버튼 이벤트 설정
            productItems.forEach((productItem) => {
                const addButton = productItem.querySelector('.action .btn-outline-dark:nth-child(1)'); // 첫 번째 버튼 (추가)
                const subtractButton = productItem.querySelector('.action .btn-outline-danger'); // 삭제 버튼 (감소)
                const productName = productItem.querySelector('.row:nth-child(2)').innerText.trim(); // 상품 이름 가져오기
            ...
            function updateProductQuantity(productName, change) {
                ...
                // Summary 섹션에서 해당 제품의 badge 요소 업데이트
                const summaryItems = document.querySelectorAll('.summary h6');
                summaryItems.forEach(function (summaryItem) {
                    if (summaryItem.innerText.includes(productName)) {
...
<div class="card">
    <div class="row">
        ...
        <div class="col-md-4 summary p-4">
            ...
            <c:forEach var="item" items="${lists}">
            <div class="row">
                <h6 class="p-0">${item.name}<span class="badge bg-dark text-">0개</span></h6>
            </div>
            </c:forEach>
